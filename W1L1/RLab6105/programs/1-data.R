...
## [your code here]
#Fetch the data that has Grad.rate >= to 80 from df dataset using Filter function and store it new dataset-df4 
df4 <- filter(df,Grad.Rate >=80)
#display dataset in which the above data is stored
df4
# Using select function fetch the X and Grad.Rate data from the current dataset-df4 and store it new dataset-df5
df5 <- select(df4,X, Grad.Rate)
#display dataset in which the above data is stored
df5
#To arrange the Grad Rate in descending order use arrange function
arrange(df5,desc(Grad.Rate))
...
## [your code here]
#Using group by function to group the data for Private column of df dataset and storing it in dfx dataset.
dfx <- group_by(df, Private)
# Using summarise function to calculate the max and min value of Outstate column.
dfx <- summarise(dfx, max=max(Outstate), min=min(Outstate))
#Using group by function to group the data for Private column of COLLEGE dataset and storing it in dfy dataset.
dfy <- group_by(college, Private)
# Using summarise function to calculate the max and min value of Outstate column and storing it in dfy dataset.
